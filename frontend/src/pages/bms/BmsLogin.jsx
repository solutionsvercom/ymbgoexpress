...
export default function BmsLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/bmsadmin', { replace: true });
    } catch (err) {
    ...
    <div className="min-h-screen bg-brand-charcoal flex items-center justify-center px-4 font-body">
      <div className="w-full max-w-md bg-brand-darker border border-brand-gold/15 rounded-2xl p-8 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={loading} className="w-full bg-brand-red hover:bg-brand-red/90 text-white py-3 rounded-xl text-sm font-bold disabled:opacity-60">
            {loading ? 'Signing in...' : 'Open BMS'}
          </button>
        </form>
      </div>
    </div>
