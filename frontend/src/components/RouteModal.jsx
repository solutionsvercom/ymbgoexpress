...
import BookingNowPopup from './BookingNowPopup.jsx';
...
export default function RouteModal({ route, onClose }) {
  ...
  const handleBookNow = () => {
    // Keep route modal open behind the popup; the popup provides direct call/WhatsApp booking.
    setBookingOpen(true);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      <BookingNowPopup
        ...
        message={`If you want to book your seats for ${route.from} → ${route.to}`}
        whatsappMessage={`Hello, I want to book seats from ${route.from} to ${route.to}. Bus type: ${busType === 'seater' ? 'AC Seater' : 'AC Sleeper'}.`}
      />
    </div>
  );
}
