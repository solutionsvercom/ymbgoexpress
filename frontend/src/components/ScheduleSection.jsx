      ...
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
          {schedules.map((s, i) => (
            <div
              key={i}
              className="border border-gray-100 rounded-2xl p-5 hover:shadow-lg transition-all h-full flex flex-col"
            >
              <div className="flex justify-between items-start gap-3 mb-3 shrink-0">
                <div className="min-w-0 flex-1">
                  <h3 className="font-bold text-[#0D5C63] leading-snug">{s.title}</h3>
                  <p className="text-xs text-gray-500 mt-0.5">{s.subtitle}</p>
                </div>
                <span className="bg-[#0D7377]/10 text-[#0D7377] text-xs px-2 py-1 rounded-full font-medium shrink-0 whitespace-nowrap">
                  {s.type}
                </span>
              </div>
              <div className="space-y-2 shrink-0">
                {s.stops.map((stop, j) => (
                  <div key={j} className="flex items-center gap-2">
                    <div
                      className={`w-2.5 h-2.5 rounded-full shrink-0 ${j === 0 || j === s.stops.length - 1 ? 'bg-[#E8762C]' : 'bg-[#0D7377]'}`}
                    />
                    <span className="text-sm text-gray-700 truncate">{stop.city}</span>
                    <span className="text-xs text-gray-500 font-medium tabular-nums ml-auto shrink-0">
                      {stop.time}
                    </span>
                  </div>
                ))}
              </div>
              <div className="flex-1 min-h-[4.5rem]" aria-hidden="true" />
              <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-500 mb-4 shrink-0">
              ...
              <button
                ...
                className="w-full bg-[#0D7377] text-white py-2 rounded-xl text-sm font-semibold hover:bg-[#0D5C63] transition-all shrink-0"
              >
                Book This Route
              </button>
            </div>
